...
class FriendsFollowersResourcesTest extends TwitterTestBase {
    ...
    void testSocialGraphMethods() throws Exception {
        IDs ids;
        ids = twitter1.friendsFollowers().getFriendsIDs(-1);
        assertNotNull(TwitterObjectFactory.getRawJSON(ids));
        int yusuke = 4933401;
        assertIDExsits("twit4j is following yusuke", ids, yusuke);

        IDs obamaFollowers;
        obamaFollowers = twitter1.friendsFollowers().getFollowersIDs("barackobama", -1);
        ...
        assertFalse(obamaFollowers.hasPrevious());
        obamaFollowers = twitter1.friendsFollowers().getFollowersIDs("barackobama", obamaFollowers.getNextCursor());
        assertEquals(obamaFollowers, TwitterObjectFactory.createIDs(TwitterObjectFactory.getRawJSON(obamaFollowers)));
        assertNotNull(TwitterObjectFactory.getRawJSON(obamaFollowers));
        assertTrue(obamaFollowers.hasNext());
        assertTrue(obamaFollowers.hasPrevious());

        obamaFollowers = twitter1.friendsFollowers().getFollowersIDs("barackobama", -1, 100);
        assertNotNull(TwitterObjectFactory.getRawJSON(obamaFollowers));
        assertEquals(obamaFollowers, TwitterObjectFactory.createIDs(TwitterObjectFactory.getRawJSON(obamaFollowers)));
        assertTrue(obamaFollowers.hasNext());
        assertFalse(obamaFollowers.hasPrevious());

        obamaFollowers = twitter1.friendsFollowers().getFollowersIDs(813286, -1, 100);
        assertNotNull(TwitterObjectFactory.getRawJSON(obamaFollowers));
        assertEquals(obamaFollowers, TwitterObjectFactory.createIDs(TwitterObjectFactory.getRawJSON(obamaFollowers)));
        assertTrue(obamaFollowers.hasNext());
        assertFalse(obamaFollowers.hasPrevious());
        ...
        IDs obamaFriends;
        obamaFriends = twitter1.friendsFollowers().getFriendsIDs("barackobama", -1, 100);
    ...
    void testCreateDestroyFriend() throws Exception {
        User user;
        try {
            user = twitter2.friendsFollowers().destroyFriendship(id1.screenName);
        ...
        user = twitter2.friendsFollowers().createFriendship(id1.screenName, true);
        assertNotNull(TwitterObjectFactory.getRawJSON(user));
        assertEquals(user, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user)));
...
//        assertTrue(detail.isNotificationEnabled());
        try {
            twitter2.friendsFollowers().createFriendship(id2.screenName);
            fail("shouldn't be able to befrinend yourself");
        } catch (TwitterException te) {
            assertEquals(403, te.getStatusCode());
        }
        try {
            twitter2.friendsFollowers().createFriendship("doesnotexist--");
...
//            assertEquals(404, te.getStatusCode());
    ...
    void testRelationship() throws Exception {
        ...
        followsOneWay.friendsFollowers().createFriendship(id1.screenName);
        ...
        Relationship rel1 = twitter1.friendsFollowers().showFriendship(id1.screenName, followsOneWayInfo.screenName);
        ...
        rel1 = twitter1.friendsFollowers().showFriendship("@" + id1.screenName, followsOneWayInfo.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(rel1));
        assertEquals(rel1, TwitterObjectFactory.createRelationship(TwitterObjectFactory.getRawJSON(rel1)));
        ...
        Relationship rel1r = twitter1.friendsFollowers().showFriendship(followsOneWayInfo.screenName, id1.screenName);
        assertNotNull(rel1r);
        ...
        rel1r = twitter1.friendsFollowers().showFriendship(followsOneWayInfo.screenName, "@" + id1.screenName);
        assertNotNull(rel1r);
        assertFalse(rel1r.isSourceFollowedByTarget());
        assertTrue(rel1r.isSourceFollowingTarget());
        assertFalse(rel1r.isTargetFollowingSource());
        ...
        ResponseList<Friendship> friendshipList = twitter1.friendsFollowers().lookupFriendships("barakobama", followsOneWayInfo.screenName);
        ...
        try {
            if (!twitter1.friendsFollowers().showFriendship(id1.id, twitterapiID).isSourceFollowingTarget()) {
                twitter1.friendsFollowers().createFriendship(twitterapi);
            }
        ...
        try {
            if (!twitter1.friendsFollowers().showFriendship(id1.id, twitterDevID).isSourceFollowingTarget()) {
                // following an account enables retweet notification forcibly. follow him only when not following
                twitter1.friendsFollowers().createFriendship(twitterdev);
            }
        } catch (TwitterException ignore) {
        }

        Relationship updatedRelationship = twitter1.friendsFollowers().updateFriendship(twitterapi, true, true);
    ...
    void testIncomingOutgoingFriendships() throws Exception {
        ...
        IDs outgoingFriendships = twitter1.friendsFollowers().getOutgoingFriendships(-1L);
    ...
    void testFriendsFolowers() throws TwitterException {
        twitter1.friendsFollowers().getFollowersIDs(id1.id, -1L).getIDs();
        twitter1.friendsFollowers().getFriendsIDs(id1.id, -1L).getIDs();
    }

    @Test
    void testFriendsFollowersList() throws Exception {
        PagableResponseList<User> t4jfriends = twitter1.friendsFollowers().getFriendsList("t4j_news", -1L);
        PagableResponseList<User> t4jfriends2 = twitter1.friendsFollowers().getFriendsList(72297675L, -1L);
        assertEquals(t4jfriends, t4jfriends2);
        assertTrue(t4jfriends.size() > 0);

        PagableResponseList<User> t4jfriends1 = twitter1.friendsFollowers().getFriendsList("@t4j_news", -1L, 3);
        PagableResponseList<User> t4jfriends12 = twitter1.friendsFollowers().getFriendsList(72297675L, -1L, 3);
        assertEquals(t4jfriends1, t4jfriends12);
        assertTrue(t4jfriends1.size() > 0);
        assertTrue(t4jfriends1.size() <= 3);

        PagableResponseList<User> t4jfollowers = twitter1.friendsFollowers().getFollowersList("t4j_news", -1L);
        PagableResponseList<User> t4jfollowers2 = twitter1.friendsFollowers().getFollowersList(72297675L, -1L);
        assertEquals(t4jfollowers, t4jfollowers2);
        assertTrue(t4jfollowers.size() > 0);

        PagableResponseList<User> t4jfollowers1 = twitter1.friendsFollowers().getFollowersList("@t4j_news", -1L, 3);
        PagableResponseList<User> t4jfollowers12 = twitter1.friendsFollowers().getFollowersList(72297675L, -1L, 3);
    ...
    void testFriendsFollowersListWithSkipStatusIncludeUserEntities() throws Exception {
        PagableResponseList<User> t4jfriends = twitter1.friendsFollowers().getFriendsList("t4j_news", -1L, 3, true, false);
        PagableResponseList<User> t4jfriends2 = twitter1.friendsFollowers().getFriendsList(72297675L, -1L, 3, true, false);
        assertEquals(t4jfriends, t4jfriends2);
        assertTrue(t4jfriends.size() > 0);
        ...
        PagableResponseList<User> t4jfollowers = twitter1.friendsFollowers().getFollowersList("t4j_news", -1L, 3, true, false);
        PagableResponseList<User> t4jfollowers2 = twitter1.friendsFollowers().getFollowersList(72297675L, -1L, 3, true, false);
        assertEquals(t4jfollowers, t4jfollowers2);
        assertTrue(t4jfollowers.size() > 0);
