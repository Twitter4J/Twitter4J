...
//        assertNotNull(user.getTimeZone());
        ...
        assertEquals(user, twitter1.v1().users().showUser("@yusuke"));
        ...
        user = twitter1.v1().users().showUser("twit4jnoupdate");
        assertNull(user.getProfileBannerURL());
        user = twitter1.v1().users().showUser("tigertest");
        User previousUser = user;
        assertNotNull(TwitterObjectFactory.getRawJSON(user));

        user = twitter1.v1().users().showUser(numberId);
        ...
        user = twitter1.v1().users().showUser(numberIdId);
        assertEquals(numberIdId, user.getId());
        assertNotNull(TwitterObjectFactory.getRawJSON(user));
        assertEquals(user, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user)));
    }
    ...
    void testLookupUsers() throws TwitterException {
        ResponseList<User> users = twitter1.v1().users().lookupUsers(id1.screenName, id2.screenName);
        assertEquals(2, users.size());
        assertContains(users, id1);
        assertContains(users, id2);

        users = twitter1.v1().users().lookupUsers(id1.id, id2.id);
        assertEquals(2, users.size());
        assertContains(users, id1);
        assertContains(users, id2);
    ...
    void testSearchUser() throws TwitterException {
        ResponseList<User> users = twitter1.v1().users().searchUsers("Doug Williams", 1);
    ...
    void testBanner() throws Exception {
        twitter1.v1().users().updateProfileBanner(getRandomlyChosenFile(banners));
        User user = twitter1.v1().users().verifyCredentials();
        if (user.getProfileBannerURL() != null) {
            twitter1.v1().users().removeProfileBanner();
        }
    }

    @Test
    void testAccountMethods() throws Exception {
        twitter1.v1().users().updateAccountSettings(1 /* GLOBAL */, true,
                "23", "08", "Rome", "en");
        User original = twitter1.v1().users().verifyCredentials();
        ...
        User altered = twitter1.v1().users().updateProfile(
        ...
        twitter1.v1().users().updateProfile(original.getName(), original.getURL(), original.getLocation(), original.getDescription());
        ...
        AccountSettings settings = twitter1.v1().users().getAccountSettings();
        ...
        AccountSettings intermSettings = twitter1.v1().users().updateAccountSettings(1 /* GLOBAL */, true,
                "23", "08", "Helsinki", "it");
        ...
        AccountSettings lastSettings = twitter1.v1().users().updateAccountSettings(1118370/*Tokyo*/, settings.isSleepTimeEnabled(),
    ...
    void testAccountProfileImageUpdates() throws Exception {
        User user = twitter1.v1().users().updateProfileImage(new FileInputStream(getRandomlyChosenFile()));
        assertNotNull(TwitterObjectFactory.getRawJSON(user));
    }
...
//            "src/test/resources/t4j-banner.gif",
    ...
    void testBlockMethods() throws Exception {
        twitter1.v1().users().createBlock(twit4jblockID);
        twitter2.v1().users().createBlock(twit4jblockID);

        User user1 = twitter2.v1().users().createBlock(id1.screenName);
        ...
        User user2 = twitter2.v1().users().destroyBlock(id1.screenName);
        ...
        user1 = twitter2.v1().users().createBlock("@" + id1.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(user1));
        assertEquals(user1, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user1)));
        user2 = twitter2.v1().users().destroyBlock("@" + id1.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(user2));
        assertEquals(user2, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user2)));

        PagableResponseList<User> users = twitter1.v1().users().getBlocksList();
        assertNotNull(TwitterObjectFactory.getRawJSON(users));
        assertEquals(users.get(0), TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(users.get(0))));
        assertTrue(users.size() >= 1);
        assertEquals(twit4jblockID, users.get(0).getId());

        IDs ids = twitter1.v1().users().getBlocksIDs();
        ...
        ids = twitter1.v1().users().getBlocksIDs(-1);
        assertTrue(ids.getIDs().length > 0);
    }

    @Test
    void testMuteMethods() throws Exception {
        User user1 = twitter2.v1().users().createMute(id1.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(user1));
        assertEquals(user1, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user1)));
        User user2 = twitter2.v1().users().destroyMute(id1.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(user2));
        assertEquals(user2, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user2)));

        user1 = twitter2.v1().users().createMute("@" + id1.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(user1));
        assertEquals(user1, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user1)));
        try {
            twitter2.v1().users().destroyMute("@" + id1.screenName);
//            assertNotNull(TwitterObjectFactory.getRawJSON(user2));
//            assertEquals(user2, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user2)));
...
//          The request with '@'+screen_name could not make mute user.
        ...
        twitter1.v1().users().createMute(twit4jblockID);
        PagableResponseList<User> users = twitter1.v1().users().getMutesList(-1L);
        assertNotNull(TwitterObjectFactory.getRawJSON(users));
        assertEquals(users.get(0), TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(users.get(0))));
        assertEquals(1, users.size());
        assertEquals(twit4jblockID, users.get(0).getId());

        IDs ids = twitter1.v1().users().getMutesIDs(-1L);
        assertNull(TwitterObjectFactory.getRawJSON(users));
        assertNotNull(TwitterObjectFactory.getRawJSON(ids));
