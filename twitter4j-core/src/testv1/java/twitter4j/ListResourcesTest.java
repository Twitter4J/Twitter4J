...
class ListResourcesTest extends TwitterTestBase {
    ...
    void testListMethods() throws Exception {
        ...
        userLists = twitter1.list().getUserLists(id1.screenName);
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        ...
        userList = twitter1.list().showUserList(userList.getId());
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertNotNull(userList);

        userList = twitter1.list().showUserList(id1.id, userList.getSlug());
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertNotNull(userList);

        List<Status> statuses = twitter1.list().getUserListStatuses(userList.getId(), Paging.empty);
        if (statuses.size() > 0) {
            assertEquals(statuses.get(0), TwitterObjectFactory.createStatus(TwitterObjectFactory.getRawJSON(statuses.get(0))));
        }
        statuses = twitter1.list().getUserListStatuses(userList.getId(), Paging.empty);
        if (statuses.size() > 0) {
            assertEquals(statuses.get(0), TwitterObjectFactory.createStatus(TwitterObjectFactory.getRawJSON(statuses.get(0))));
        }
        ...
        userList = twitter1.list().updateUserList(userList.getId(), "testpoint2", true, "description2");
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        ...
        userList = twitter1.list().updateUserList(id1.id, userList.getSlug(), "testpoint3", true, "description3");
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertTrue(userList.isPublic());
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertNotNull(userList);
    ...
    void testListMemberMethods() throws Exception {
        ...
        /*List Member Methods*/
        User user;
        userList = twitter1.list().createUserListMember(userList.getId(), id2.id);
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        userList = twitter1.list().createUserListMembers(userList.getId(), id3.id);
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        userList = twitter1.list().createUserListMembers(userList.getId(), new String[]{"yusukey", "yusuke"});
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertNotNull(userList);

        PagableResponseList<User> users = twitter1.list().getUserListMembers(userList.getId(), -1);
        ...
        users = twitter1.list().getUserListMembers(userList.getId(), -1);
        assertEquals(users.get(0), TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(users.get(0))));
        assertNotNull(TwitterObjectFactory.getRawJSON(users));
        assertEquals(4, users.size());

        userList = twitter1.list().destroyUserListMember(userList.getId(), id2.id);
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(userList);
        ...
        user = twitter1.list().showUserListMembership(userList.getId(), id3.id);
        ...
        userLists = twitter1.list().getUserListMemberships(id1.screenName, -1L);
        ...
        userLists = twitter1.list().getUserListSubscriptions(id1.screenName, -1L);
        assertNotNull(TwitterObjectFactory.getRawJSON(userLists));
        if (userLists.size() > 0) {
            assertEquals(userLists.get(0), TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userLists.get(0))));
        }
    ...
    void testRemoveListMembers() throws Exception {
        ...
        userList = prepareListTest();
        try {
            twitter1.list().showUserListMembership(userList.getId(), id2.id);
        ...
        userList = twitter1.list().destroyUserListMember(userList.getId(), id2.screenName);
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(userList);
        userList = twitter1.list().showUserList(userList.getId());
        ...
        userList = twitter1.list().createUserListMembers(userList.getId(), screenNames);
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));

        twitter1.list().getUserListMembers(userList.getId(), -1);

        // Remove 2 by screen name
        userList = twitter1.list().destroyUserListMembers(userList.getUser().getScreenName(), userList.getSlug(), screenNames);
        userList = twitter1.list().showUserList(userList.getId());
        ...
        userList = twitter1.list().createUserListMembers(userList.getId(), userIds);
        assertEquals(userList, TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userList)));
        assertNotNull(TwitterObjectFactory.getRawJSON(userList));
        ...
        // Remove 2 by screen name
        userList = twitter1.list().destroyUserListMembers(userList.getId(), userIds);
        twitter1.list().showUserList(userList.getId());
    }
    ...
    void testListSubscribersMethods() throws Exception {
        ...
        users = twitter1.list().getUserListSubscribers(id1.id, userList.getSlug(), -1L);
        assertNotNull(TwitterObjectFactory.getRawJSON(users));
        twitter2.list().createUserListSubscription(userList.getId());
        twitter2.list().destroyUserListSubscription(userList.getId());
        users = twitter1.list().getUserListSubscribers("twitterapi", "team", -1L);
        assertTrue(0 < users.size());
        User user;
        user = twitter1.list().showUserListSubscription("twitterapi", "team", 4933401);
        assertNotNull(TwitterObjectFactory.getRawJSON(user));
        assertEquals(user, TwitterObjectFactory.createUser(TwitterObjectFactory.getRawJSON(user)));
        assertEquals("yusuke", user.getScreenName());
        userLists = twitter1.list().getUserListSubscriptions("yusuke", -1L);
        assertNotNull(userLists);
        if (userLists.size() > 0) {
            assertEquals(userLists.get(0), TwitterObjectFactory.createUserList(TwitterObjectFactory.getRawJSON(userLists.get(0))));
        }
        ...
        try {
            userList = twitter1.list().destroyUserList(userList.getId());
    ...
    void testUserListsOwnerships() throws Exception {
        ...
        lists = twitter1.list().getUserListsOwnerships("t4jtest123", 3, -1);
        assertTrue(lists.size() > 0);
        lists = twitter1.list().getUserListsOwnerships(4933401L, 3, -1);
        assertTrue(lists.size() > 0);
    ...
    private UserList prepareListTest() throws Exception {
        ...
        for (UserList alist : userLists) {
            if (alist.getSlug().equals("testpoint1")) {
                ...
                try {
                    UserList deletedLiet = twitter1.list().destroyUserList(id1.id, alist.getSlug());
        ...
        if (list == null) {
            list = twitter1.list().createUserList("testpoint1", true, "description1");
        }
        Thread.sleep(5000);
        return list;
    }
