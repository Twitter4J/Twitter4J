 ...
 */
public class HTMLEntityString {
	/**
	 * Converts indexes in an HTML-escaped string from the domain of one
	 * {@link twitter4j.internal.http.Escapedness} into another {@code Escapedness}.
		...
		public IndexMapper(int length, List<Entity> entities, Escapedness fromEscapedness, Escapedness toEscapedness) {
			...
			for(int fi=0;fi<length;fi++,ti++) {
				if(entity != null) {
					...
					if(fi>fromEscapedness.getEntityStart(entity) && fi<fromEscapedness.getEntityEnd(entity))
						this.conversion[fi] = -1;
					else
					if(fi == fromEscapedness.getEntityEnd(entity)) {
	...
	public static enum Escapedness {
		ESCAPED {
			...
			public String getTokenAtPosition(StringBuffer s, int index) {
				String result;
				...
				if(s.charAt(index) == '&') {
					...
					if(semicolon != -1) {
						...
						if(escapeEntityMap.containsKey(substring))
							result = substring;
						else
							result = null;
					}
			...
			public String escapeToken(Escapedness escapedness, String entity) {
				...
				if(escapedness == UNESCAPED)
					result = escapeEntityMap.get(entity); 
				else
		...
		UNESCAPED {
			...
			public String getTokenAtPosition(StringBuffer s, int index) {
				...
				if(entityEscapeMap.containsKey(ch))
					result = ch;
				else
					result = null;
			...
			public String escapeToken(Escapedness escapedness, String entity) {
				...
				if(escapedness == ESCAPED)
					result = entityEscapeMap.get(entity); 
				else
					throw new IllegalArgumentException("Invalid escapedness: "+escapedness);
		 ...
		 *         &amp;amp; to &amp;).
		 */
		public abstract String getTokenAtPosition(StringBuffer s, int index);
    ...
    public static HTMLEntityString escape(StringBuffer s) {
    ...
    public static HTMLEntityString unescape(StringBuffer s) {
    	return convert(s, Escapedness.ESCAPED, Escapedness.UNESCAPED);
    }
    
    public static HTMLEntityString convert(StringBuffer s, Escapedness fromEscapedness, Escapedness toEscapedness) {
	...
	private StringBuffer convertedText;
	...
	public StringBuffer getConvertedText() {
		return convertedText;
	}
	
	public void setConvertedString(StringBuffer convertedText) {
